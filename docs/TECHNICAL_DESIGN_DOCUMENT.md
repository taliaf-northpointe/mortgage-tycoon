...
- **v13 → v14 + office polish (2026-07-08):** v14 renames duplicate employee names from before the uniqueness fix (first keeps the name; twins take an unused candidate name gender-matched to their sprite via `SPRITE_GENDER`). Candidates carry a one-line `about` bio (`CANDIDATE_ABOUT`, 32 entries) shown in the hire modal and kept on `Employee.about` (optional field). Office scene: sprites are head-width normalized (`headWidth` median + `TARGET_HEAD_WIDTH` 165 in process-art.mjs — canvas-only normalization left tight face-crops looking huge); the desk grid seats 4/5/6 per row by team size and scales exactly into the floor (top ≤ wall line, bottom anchored to the platform, centered).
- **v12 → v13** (2026-07-08, three playtest fixes): (1) backfills upgrade ids added after a save was created — an absent entry read as "buy the previous tier first" even with the whole track owned; the backfill is generic (available when the previous tier is purchased), so future tree growth only needs a version bump. (2) Rewrites the repetitive "keys and kettle" fallback notes: the shared pool grew to ~50 voice-tagged notes and the single fallback became 13 name-embedded templates. (3) Notes are now voice-aware — `SHARED_NOTES`/`FALLBACK_TEMPLATES` carry `voice: 'we' | 'i' | 'any'` and `thankYouNote` filters by `PORTRAIT_KIND` (now exported from leads.ts), so a solo buyer never signs a "we" note. Separately (no save change): `normalizeBust` in process-art.mjs puts every office character on a uniform 300×180 canvas, so the office scene renders all faces at one size.
...
- **Office 4 + upgrade tiers 6–7 (2026-07-08, no save change):** the upgrade tree grows to 5 × 7 = 35 (`content/upgrades.ts`; `initialUpgradeStates` still opens tier 1 only). Level gates re-spaced: tiers 4–5 moved from level 5 to `UPGRADES_TIER45_LEVEL` 8, new tiers 6–7 behind `UPGRADES_TIER67_LEVEL` 18 (both enforced in `purchaseBlockedReason`, each with a just-in-time training pop-up). `OFFICE_ART_STAGES` is 4: office tiers 6–7 earn Talia's Office 4/Desk 4 art (skyline penthouse, canvas 1402×1122 in `OfficeScene.STAGE_CANVAS`). Dashboard layout: the scene's 640px height cap is gone and the side columns slimmed (210/1fr/300) so the office fills the stage area.
